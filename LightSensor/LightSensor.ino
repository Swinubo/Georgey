...
int SENSOR_PIN = 13  // Connect the KY-033 OUT pin to Arduino digital pin 13

void setup() {

  Serial.begin(9600);
  Serial.println("Program has begun.");
  pinMode(SENSOR_PIN, INPUT);  // Configure the sensor pin as input
...
void loop() {
  int sensorState = digitalRead(SENSOR_PIN);  // Read the sensor's digital output

  if (sensorState == LOW) {
    Serial.println("LED ON - Black line detected!");  // LED is ON, black surface
  } else {
    Serial.println("LED OFF - White surface detected!");  // LED is OFF, white surface
    stop();
  }
